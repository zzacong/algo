...
import { TREE_ALGORITHMS, type TreeAlgorithm, type TreeFrame } from "@/data/trees";

export const Route = createFileRoute("/trees/")({
  component: TreesGalleryPage,
});

const FRAMES_CACHE = new Map<string, TreeFrame[]>();

function getFrames(algo: TreeAlgorithm): TreeFrame[] {
...
function AlgorithmCard({ algo }: { algo: TreeAlgorithm }) {
  ...
  return (
    ...
    >
      ...
      <div className="border-b border-border bg-muted/40 px-3 pt-3 pb-2">
        <TreeVisualizer frames={frames} fps={5} size="sm" />
      </div>
      ...
      <main id="main-content">
        ...
        <div className="mx-auto max-w-5xl px-4 pt-12 pb-8 sm:px-6 sm:pt-16">
          ...
          <p className="mt-4 max-w-xl text-sm leading-relaxed text-pretty text-muted-foreground">
            Watch insert, search, delete, and rotation operations play out on a binary search tree —
            node by node. Click any card to explore the full animation.
          </p>
        </div>
